...
## Usage
```yaml
name: Build and Push Python Image to Aws CI on ECR
  ...
  deploy:
    ...
    steps:
      ...
      - name: Configure kubectl
        run: |
          aws eks --region us-east-1 update-kubeconfig --name test-eks-test-cluster

      - name: 'Deploy to eks'
        env:
          IMAGE_TAG: latest #$(git rev-parse HEAD)
        run: helm upgrade -i suresh --create-namespace -n suresh --set image.tag=latest ./test -f ./test/values.yaml
